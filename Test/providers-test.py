...
# 跳转到商家中心页面进行以下操作
from Test.providers_Login import Login  # 调用登陆函数

sleep(2)
Login().user_login(driver)
sleep(2)
for i in range(5):  # 打开商家中心
    try:
        el = driver.find_element_by_link_text('商家中心')
        if el.is_displayed():
            break
...
sleep(1)
Select(driver.find_element_by_id('providers_rst')).select_by_index('4')
sleep(2)
...
# driver.execute_script('arguments[0].scrollIntoView();', target)
# sleep(2)
# Select(driver.find_element_by_id('providers_rst_241')).select_by_index('1')
